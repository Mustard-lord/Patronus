...
def clip_score_pre_ckpt(ckpt, vae, clip_path, n_samples, outdir):
    from torchmetrics.functional.multimodal import clip_score
    ...
    # prompt_path = '../../code/imagenet-autoencoder-main/list/2kimagenettrainprompt.txt'
            ...
            with model.ema_scope():
                ...
                for n in trange(n_iter, desc="Sampling"):
                    ...
                    for prompts in tqdm(data, desc="data"):
                        ...
                        #         os.path.join(f"./{base_count:05}.jpg"))
                        #     base_count += 1
                        # import pdb;pdb.set_trace()
                        clip_score_batch = clip_score_fn((255*x_samples_ddim).int(), prompts).detach()
                        clip_score_final.append(clip_score_batch)
                        print(clip_score_final)
                    clip_all = torch.cat(clip_score_final, dim=0)
                    average_clip_score = clip_all.mean()
                    std = clip_all.std()
                    # average_clip_score = torch.mean(torch.tensor(clip_score_final).to(torch.float)) 
                    return average_clip_score, std
