...
# from clip_score_pre_ckpt import clip_score_pre_ckpt
from cal_clip.clip_score_pre_iteration import *
...
def test_finetune(save_dir, model, tar_finetuneloader, tar_testloader, epochs, lr, optimizer_select, baseline, finetune_number):
    ...
    ckpt = '../../code/diffusion/miniSD.ckpt'
    n_samples = 10
    average_clip_score_list = []
    std_list = []
    ...
    for ep in tqdm(range(epochs)):
        ...
        with tqdm(tar_finetuneloader) as tqdm_iter:
         for inputs in tqdm_iter:
            ...
            if count%10==0:              
                average_clip_score, std = clip_score_pre_ckpt(ckpt, model, clip_path, n_samples, outdir)
                average_clip_score_list.append(average_clip_score.item())  
                std_list.append(std.item())
                os.makedirs(f'./epochs_2_500_seed_{args.seed}', exist_ok=True)
                clip_score_dict = {'clip_score': average_clip_score_list,  'std': std_list }
                clip_score_df = pd.DataFrame(clip_score_dict)
                clip_score_df.to_csv(f'./epochs_2_500_seed_{args.seed}/clip_score_{baseline}_{optimizer_select}_batch_size_{inputs.shape[0]}_lr_{lr}_finetune_size_{finetune_number}_epochs_{epochs}.csv', index=False)
            
            
            # res.append(test_loss)
            # loss_df = pd.DataFrame()
            # loss_df['mse_loss'] = res
            # loss_df.to_csv(f'./mse_loss_{baseline}_{optimizer_select}_batch_size_{inputs.shape[0]}_lr_{lr}_finetune_size_{finetune_number}.csv', index=False)
...
def main(args):
    ...
    set_seed(args.seed)
    save_path = args.root + "_500_clip"+f'/{args.baseline}/'
    now = datetime.now()
