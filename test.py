...
def main():
    ...
    now = datetime.now()
    if opt.vae:
        ckpt_path = os.path.dirname(opt.vae)
        opt.outdir = ckpt_path + '/../eval_sd' + '/' + f'{now.month}_{now.day}_{now.hour}_{now.minute}_{now.second}/' + opt.outdir
        os.makedirs(opt.outdir, exist_ok=True)
        outpath = opt.outdir + '/images'
        os.makedirs(outpath, exist_ok=True)
    else:
        outpath = './test_t2i_new_step0'
        os.makedirs(outpath, exist_ok=True)

    # with open(opt.outdir + '/prompt.txt', "w") as file:
    #     file.write(opt.prompt)
    # with open(opt.outdir + '/ckpt.txt', "w") as file:
    #     file.write(opt.vae)
    ...
    bias = 0
    base_count = len(os.listdir(sample_path)) + bias
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                if not opt.skip_grid:
                    ...
                    Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, f'{opt.prompt.replace(" ", "-")}-{grid_count:04}.jpg'))
                    grid_count += 1
