...
class MyAutoencoderKL(pl.LightningModule):
    def __init__(self,
                 ddconfig,
                 lossconfig,
                 embed_dim,
    ...
    def get_input(self, batch, k):
        # x = batch[k]
        x, target, flag = batch[0], batch[1], batch[2]
        if len(x.shape) == 3:
            x = x[..., None]
        # x = x.permute(0, 3, 1, 2).to(memory_format=torch.contiguous_format).float()
        # target = target.permute(0, 3, 1, 2).to(memory_format=torch.contiguous_format).float()
